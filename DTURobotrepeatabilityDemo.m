...
for d = 1:numel(featExtractors)
  for imgNo = 1:1 %dataset.NumImagesPerPerturbation
    for confNo = 1:dataset.NumPerturbationsConfigurations
      img_ref_id = dataset.getReferenceImageId(confNo, imgNo);
      img_id = dataset.getImageId(confNo, imgNo);
      
      [repeatability(d, confNo, imgNo) numCorresp(d, confNo, imgNo)] = ...
...
featExtractors = {thrSiftDetector};%, siftDetector, mserWithSift};
detectorNames = {'SIFT PT=10'};%,'SIFT','MSER with SIFT'};
...
matchingBenchmark = DTURobotRepeatabilityBenchmark('Mode','MatchingScore');
...
for d = 1:numel(featExtractors)
  for imgNo = 1:1%dataset.NumImagesPerPerturbation
    for confNo = 1:dataset.NumPerturbationsConfigurations
      ...
      [matchScore(d, confNo, imgNo) numMatches(d, confNo, imgNo)] = ...
