...
% AUTORIGHTS
  ...
  methods (Access=protected)
    ...
    function compile(obj)
      ...
      % Run Make
      status = helpers.osExec(OpenCVInstaller.BuildDir, ...
        OpenCVInstaller.MakeCommand,'-echo');
  ...
  methods (Static)
    ...
    function mexflags = getCompileFlags()
    % getMexFlags Get flags for compilation of mex files
    %   MEX_FLAGS = getMexFlags() Returns mex flags needed for compilation
    %   of mex files which link to OpenCV library.
      import helpers.*;
      switch computer()
        case {'GLNX86','GLNXA64'}
        mexflags = sprintf(...
          '-Wl,-rpath,%s -L%s -lopencv_core -lopencv_imgproc -lopencv_features2d -lopencv_contrib -lopencv_highgui -lopencv_nonfree -I%s',...
          OpenCVInstaller.LibDir,OpenCVInstaller.LibDir,...
          OpenCVInstaller.IncludeDir);
        otherwise
          warning('Architecture not supported yet.');
