function result = fastEllipseOverlaps(f1, f2, varargin)
% FASTELLIPSEOVERLAP Compute overlaps of two sets ov ellipses
%  EVAL = MATCHELLIPSES(F1, F2) Computes ellipse overlaps between 
%  each ellipses from F1 to each ellipse in F2.
%  EVAL output structure of size(F2,2) contains following values:
%  
%    EVAL.NEIGH : for each F2 frame, the list of F1 neighbors
%    EVAL.SCORES : for each F2 frame, relative overlap of F1 neighbours
%
